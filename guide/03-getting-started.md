# 03 · Getting Started — Setup, the `shadowDom` rule, and folder structure

This page gets you to the point where you can actually customize. It assumes the decision tree ([`02-decision-tree.md`](./02-decision-tree.md)) has told you which layer(s) you're using.

> 🟢 **You're working on an already‑set‑up project.** This guide (and the AI plugin) runs on a codebase where **Velt is already installed, the user is already authenticated, and comments/sidebar/etc. are already rendering**. **Your job is customization only** — add wireframes, add/compose primitives, add CSS. You do **not** set up the SDK, auth, or documents; assume they exist and work. The one exception is the **primitives** path: there you fetch the data with hooks and loop over it to render — but that data plumbing is part of *building the customization*, not project setup. Section 1 below is **context only** (so you recognize what's already in the project); skip straight to §2 if the app already shows Velt's default UI.
...
2. **Authenticate the user via an auth provider** — pass an **`authProvider`** to `<VeltProvider>`. This is the production way to sign a user in: the auth provider carries the `user` object **and** a `generateToken` callback that returns a Velt JWT (the SDK calls it on sign‑in and refreshes it automatically on expiry). The token is minted by *your* backend (which holds your Velt API secret), so users can't spoof identity.
   ...
   ```
   `VeltAuthProvider` = `{ user, generateToken?, retryConfig?, options?, onError? }`. *(There's also a simpler `useIdentify()` → `identify(user)` for quick prototyping without tokens, but **prefer the auth provider** — it's what production setups use and what the reference implementations use.)*
3. **Set the document** — `const { setDocuments } = useSetDocuments();` then `setDocuments(documents, options)` so there's a place for comments to live. (Note the setter pattern — it's **not** `useSetDocuments([...])`.)
...
**Rule of thumb:** anything beyond pure variable theming ⇒ shadow DOM off **or** `injectCustomCss`. (See [`approaches/css.md`](./approaches/css.md) and [`rules.md`](./rules.md).)
...
These come up in every approach; the full list is in [`rules.md`](./rules.md). The two that affect setup:

1. **Use one `<VeltWireframe>` in the whole app.** It feeds a global registry; extra roots merge first‑with‑content‑wins (order‑dependent, conflict‑prone). Put one in `VeltCustomization.tsx`, rendered once near the root. (See R1 in [`rules.md`](./rules.md).)
...
These apply across every approach — handle them as you build, not after. Quick version below; **full guidance in [`cross-cutting.md`](./cross-cutting.md).**
...
- **Responsive / mobile.** Use the layout props (`filterPanelLayout="bottomSheet"`, `embedMode`, the dialog's bottom‑sheet mode) plus your own CSS media queries on your wrappers/wireframe markup. See [`reference/component-config.md`](./reference/component-config.md).
- **Testing / verifying a customization.** Per surface: compare against Velt's default (remove your customization); check the empty / loading / unread / resolved states; check dark mode; check keyboard + scroll; resize for mobile. For automated tests, assert on Velt's **stateful classes** ([`reference/css-classes.md`](./reference/css-classes.md)) or read data via **hooks** ([`reference/hooks.md`](./reference/hooks.md)).
...
- **Look up, don't guess.** Every CSS variable, wireframe slot, variable token, and hook is listed in [`reference/`](./reference). If it's not there, it doesn't exist — don't invent names.
...
➡️ Now open the file for your chosen layer: [`approaches/css.md`](./approaches/css.md) · [`approaches/primitives.md`](./approaches/primitives.md) · [`approaches/wireframes.md`](./approaches/wireframes.md) · [`approaches/headless.md`](./approaches/headless.md) — or [`approaches/combining-approaches.md`](./approaches/combining-approaches.md) if you're mixing.
