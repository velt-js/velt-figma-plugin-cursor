...
**Progressive escalation.** Start at the cheapest layer; escalate only when blocked: CSS → Wireframes → Primitives → Headless. Never start at headless "to be safe" — it's the most expensive. See [`02-decision-tree.md`](./02-decision-tree.md).

**Per‑feature layering.** Decide approach **per surface**; theme everything with one CSS file. Typical healthy mix: wireframe the dialog, primitive sidebar, headless badge in your header, CSS on top. See [`approaches/combining-approaches.md`](./approaches/combining-approaches.md).
...
**Look up every name.** CSS var / class / slot / `{…}` / prop / hook → it's in [`reference/`](./reference) or it doesn't exist. The reference map: [`wireframe-components.md`](./reference/wireframe-components.md) (slots), [`wireframe-variables.md`](./reference/wireframe-variables.md) (tokens), [`css-classes.md`](./reference/css-classes.md) (stateful classes), [`component-config.md`](./reference/component-config.md) (props).
...
**Bridge a wireframe button to your app logic (the `onClick` escape hatch).** You can't use `onClick` inside a wireframe (R4). For a *custom* action, use **`VeltButtonWireframe`** + the **`veltButtonClick`** event:
```tsx
...
**Customize a list row, not the list.** List/repeater slots ignore custom layout — restructure the repeated **item** wireframe instead. See [`approaches/wireframes.md`](./approaches/wireframes.md) §7b.

**Scope deliberately.** Nest a child wireframe inside its parent to scope it to that parent's render; place it at the `<VeltWireframe>` root to apply globally. See [`approaches/wireframes.md`](./approaches/wireframes.md) §3b.
...
**Custom dropdown — your UI library + Velt primitive items + wireframe‑styled items.** Build the header dropdown (status / priority / options) from *your* UI‑library shell, Velt **primitive** dropdown items (which carry the set behavior), and a **wireframe** to style the item internals — or go fully headless with `useUpdateStatus()` / `useUpdatePriority()`. Full worked example with the exact component names: [`approaches/primitives.md`](./approaches/primitives.md) → "custom dialog header with a custom dropdown."

**Custom sidebar filters (V2) — add your own filters + restyle the filter dropdown/panel.** Configure which filters show (and add `type:'custom'` filters with your own options) via the `filters` prop; pick the layout with `filterPanelLayout` / `filterOptionLayout`; and fully restructure the filter dropdown/panel via the `FilterContainer` / `FilterDropdown` / `FilterButton` wireframe slots — Velt keeps the filtering logic. Full details: [`reference/component-config.md`](./reference/component-config.md) → "Sidebar filters."
