...
> **Golden rule of verification: measure the STYLE of every element, not the SCREENSHOT of the surface — and name every difference.** The design is **DUMMY data**; the live app has **REAL data** (2 comments vs 11, "example comment" vs "Hello", made-up names/timestamps vs real users) — so they never match pixel-for-pixel, and a whole-surface pixel comparison against the mock is **not a valid gate**. Fidelity is a **content-independent, per-element** problem: you verify the *look of each template element* — which is true whether the app shows 2 cards or 200. Two failures came from the opposite mistakes: "looks ~90% close" (eyeballing) and "5 sampled properties pass" while the surface was wrong (thin sampling). So you check, and ALL must be clean: (1) **style** — rendered computed styles vs the `designSpec`'s exact numbers, per element, per property (non-structural lengths **±2px**, colour **CIEDE2000 ΔE < 2**, keywords exact, font-family by family); (2) **layout** — each element's surface-relative **box** (x/y/w/h), sibling **gaps** (esp. the **inter-card gap** — the count varies with real data, the gap does not), and **relations** (name left-of time, message below header, actions top-right), plus **missing/extra elements**. The checklist is **auto-derived from every mapped element** (no hand-picking — that's how a broken surface slipped through), and the gate **rejects a spec that covered too little** (INCOMPLETE). (3) A **visual side-by-side is ADVISORY** — it flags regions worth a look, but a pixel region is only a defect once you can name it as a style (1) or layout (2) row; a raw region, or a pure **CONTENT** difference (comment count, text, names, timestamps, reaction counts), is **never** a FAIL. There is **no aggregate score** and **"looks close" is a FAIL**. (Tolerances exist only because Velt's engine differs sub-pixel from a design tool — not to permit visible differences. The single mechanical exception is R29: sparse sub-pixel residue confined to a glyph whose asset identity is verified — classified by `visual-diff.mjs --accept-glyph-residuals`, recorded as accepted-with-note, never hand-waved.)
>
...
4. **Visual side-by-side — ADVISORY, not a gate:** capture the full-surface screenshot beside the Figma frame to **spot regions worth investigating** (density/spacing, name placement, filter as a box vs icon, hover actions revealed, unwanted banner). It does **not** FAIL on its own — the live app's REAL data differs from the mock's DUMMY data (2 vs 11 cards, other text), so raw pixel regions and content differences are expected, not defects. For any region that reflects a genuine *style* issue, find the property/relation you didn't measure, **add it to the delta spec (1)/(2)**, and let THAT gate it. A clean delta table plus an obviously-wrong screenshot means you're missing a style/layout assertion — add it; never fail on the pixels alone.
