...
- Need a **custom** action from wireframe UI? Use `VeltButtonWireframe` + `useVeltEventCallback("veltButtonClick")` (see [`patterns-and-tips.md`](./patterns-and-tips.md)).
- Need genuinely custom interactive components inside the UI? That's the signal for **[headless](./approaches/headless.md)** or **[primitives](./approaches/primitives.md)**.
...
- **List/repeater slots ignore custom layout.** Some slots (the comments list, presence avatar list, reactions‑panel items, activity‑log list, …) keep rendering Velt's own loop — wrapping them in your own grid/flex or reordering does nothing. Customize the repeated **item** wireframe instead, not the list container. (See [`approaches/wireframes.md`](./approaches/wireframes.md) §7b.)
...
| `velt-data` shows blank | Wrong/undefined variable or wrong slot | Check name + slot context ([tokens](./reference/wireframe-tokens.md)) |
...
| Custom dropdown items don't set status/priority | Used non‑Velt items / missing annotation context | Use Velt primitive `…DropdownContentItem` (or `useUpdateStatus`) with `annotationId` ([primitives](./approaches/primitives.md)) |
