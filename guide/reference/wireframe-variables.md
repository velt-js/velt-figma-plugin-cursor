...
| *(no `totalComments`)* | — | — | Text Comment exposes **no** comment-count token; document-wide counts live on the sidebar (`commentAnnotationsCountByFilters` / `commentAnnotationsCountByStatus`) | — | |
...
| `componentConfig.virtualScrollData` / `.commentAnnotationsCountByFilters` / `.searchQuery` / `.taggedFilters` | array/object/string | data | data (people/tag filters are `taggedFilters` — there is **no** `selectedFilterTags`) | — | |
...
| `componentConfig.isActive` *(tooltip)* | bool | app | active state (no `lastActiveAt` token — away state comes from `presenceUser.onlineStatus`) | — | 🔑 |
...
| `componentConfig.attendee` / `.color` / `.gainVolume` | object/string/number | data/ui | per tile (no `attendee.isLocal`; local-vs-remote is `!componentConfig.isRemoteStream`) | — | |
...
| `totalTimeValue` / `currentTimeValue` / `recordedTime` | number | data/state | durations (the elapsed value is `recordedTime` — there is **no** `elapsedTime` token) | — | |
