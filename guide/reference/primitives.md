...
> **Source of truth — exhaustive for what it covers.** Generated from the Velt SDK (`@veltdev/react`). **If a component isn't on this page, it doesn't exist** — don't import it. Never invent or guess a name; use only identifiers that appear verbatim here or that you verified against SDK source and wrote back to this bar. *Scope: the complete React primitive (non‑wireframe) component surface; each composes over the SDK's `velt-*` custom elements.*
