...
`<VeltComments>` is a single host component with ~122 active props. Its exhaustive per-prop behavior is grouped by prop family below — **Core feature toggles**, **Mentions / autocomplete**, **Layout / interaction modes**, **Placeholders**, **Shadow DOM & dark mode**, and **Callbacks** — followed by the **deprecated aliases**. Each row states the prop's default, its runtime effect, and how it interacts with related props. The zero-prop baseline is summarized in [Default behaviors (no prop needed)](#default-behaviors-no-prop-needed); every prop in [`props.md`](../props.md) Part 1 is covered (see [Coverage](#coverage)).
