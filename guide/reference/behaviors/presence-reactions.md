...
- **The default emoji set.** With no customization the picker shows Velt's built-in SVG emoji set — confirmed keys: **`RAISED_HANDS`, `THUMBS_DOWN`, `THUMBS_UP`, `HEART_FACE`, `TEARS_OF_JOY`, `EYES`, `FIRE`**. The exact rendered ordering follows the emoji-map iteration order (unconfirmed as a guaranteed/stable display order). **These same keys are the default COMMENT reaction ids too** — valid as `reactionId`/`excludeReactionIds` values on the comment thread-card's `ReactionPin`/`Reactions` slots; see [`features/reactions.md`](../../features/reactions.md) §Default reaction ids (don't re-derive them from the live DOM — they're not exposed there).
