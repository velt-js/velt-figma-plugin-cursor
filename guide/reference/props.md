...
Two small utility components for binding/rendering live data and conditionally rendering markup — used inside custom pin/dialog/wireframe layouts (see [`wireframe-variables.md`](./wireframe-variables.md)). All props are optional; both also accept `className` and `id`.
...
> The **directive** form of this — `veltIf` as a prop/attribute on any wireframe slot — is documented in [`wireframe-variables.md`](./wireframe-variables.md) and [`wireframe-components.md`](./wireframe-components.md); `VeltIf` is the standalone component form.
