...
These are exported from `@veltdev/react` but belong to out-of-scope features. They are listed here only to keep the "a hook not listed doesn't exist" invariant true — name, category, return type, and one-line purpose. Return types verified from source.
