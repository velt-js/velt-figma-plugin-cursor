...
Standalone, composable parts of the comment dialog — each renders a real `velt-comment-dialog-*` live element and is an **anchored** part of the dialog (it inherits the dialog's Velt-computed position). Use them to compose a custom dialog layout from the individual controls.

**Two ways to pass `annotationId` / `commentId` (either works):**
1. **On the primitive** — pass `annotationId` (and `commentId` when the leaf is comment-scoped) directly on that component, plus the common `defaultCondition` / `inlineCommentSectionMode`.
2. **On `VeltCommentDialogContextWrapper`** — pass `annotationId` (and optional attrs like `commentId`) once on the wrapper; nested dialog primitives inherit that context. Prefer when composing several primitives for the same thread/comment. Wrapper context wins over a child's own attribute when both are set. See [`approaches/primitives.md`](../approaches/primitives.md) → *Scoping — annotationId / commentId*.
...
- `velt-comment-dialog-context-wrapper` — React: `VeltCommentDialogContextWrapper` (typed prop `annotationId`; passthrough attrs e.g. `commentId` → `comment-id`). **One of the two scoping modes** above: injects annotation (and optional comment) context into nested dialog primitives so they resolve against the right thread/comment without each child repeating the ids.
