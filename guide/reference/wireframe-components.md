...
> **Source of truth — exhaustive for what it covers.** Generated from the Velt SDK wireframe registry (`@veltdev/react`). **If a `-wireframe` slot isn't in the Appendix, it doesn't exist** — registering or filling an unregistered `-wireframe` tag renders nothing. Never invent or guess a slot name; use only tags that appear verbatim here or that you verified against SDK source and wrote back to this bar. *Scope: the Appendix aims to list every registered `velt-*-wireframe` slot element; it was extended with the verified families noted in §1 and in the Appendix sub-sections (Comment Dialog **Visibility Dropdown** standalone family, Comments Sidebar **root + filter-checkbox** slots, **Reactions root** slots). The autocomplete **panel** child elements (`velt-autocomplete-panel`, …) are live custom elements, not `-wireframe` slots, so they are intentionally not in this wireframe appendix.*
