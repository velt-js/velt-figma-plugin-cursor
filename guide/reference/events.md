...
> **Source of truth — exhaustive for what it covers.** Generated from the Velt SDK (`@veltdev/react` / core) event enums + payload maps. **If a name isn't on this page, it doesn't exist** — don't use it (a missing identifier resolves to `undefined` / renders nothing). Never invent or guess a name; use only identifiers that appear verbatim here or that you verified against ground truth and wrote back to this bar. *Scope: exhaustive per feature over the SDK's event enums + payload maps. Comments + notifications are the in-scope focus; other features (recorder, presence, live-state-sync, rewriter, suggestions, CRDT, core/resolvers) are included for completeness.*
...
| `commentNavigationButtonClick` | `CommentNavigationButtonClickEvent` (extends `CommentClickEvent`) | **Fires:** when the navigation ("go to") button on a sidebar comment is clicked. **Payload:** same shape as `commentClick`. **Gotcha:** the underlying element method `onCommentNavigationButtonClick` is commented out in source, but the event is part of `CommentEventTypesMap` and fires via `.on('commentNavigationButtonClick')`. |
